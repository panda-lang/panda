...
public class ConstructorLivingFrame extends AbstractLivingFrame<ConstructorFrame> {

    public ConstructorLivingFrame(ConstructorFrame scope) {
        super(scope);
    }

    @Override
    public void execute(Flow flow) {
        for (Cell cell : super.getFrame().getCells()) {
            if (!cell.isExecutable()) {
                continue;
            }

            Executable executable = cell.toStatement();
            flow.call(executable);
        }
    }
