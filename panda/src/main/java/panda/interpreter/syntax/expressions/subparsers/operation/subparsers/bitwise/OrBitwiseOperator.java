...
public final class OrBitwiseOperator extends BitwiseOperation {

    @Override
    public RPNOperationAction<Number> of(Type returnType, int priority, Expression a, Expression b) {
        BiFunction<Number, Number, Number> or = toFunction(priority);
    ...
    private BiFunction<Number, Number, Number> toFunction(int priority) {
        switch (priority) {
            ...
            case FLOAT:
                throw new PandaParserException("Cannot use or operator on float type");
            case DOUBLE:
                throw new PandaParserException("Cannot use or operator on double type");
            default:
