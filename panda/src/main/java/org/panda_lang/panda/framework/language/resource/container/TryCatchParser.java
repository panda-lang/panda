...
import org.panda_lang.panda.framework.language.architecture.statement.DefaultContainer;
...
public final class TryCatchParser extends ParserBootstrap {
    ...
    void parse(Context context, @Component Container container, @Src("try-body") Snippet tryBody, @Src("catch-what") Snippet catchWhat, @Src("catch-body") Snippet catchBody) throws Exception {
        Container tryContainer = containerParser.parse(context, new DefaultContainer(), tryBody);
        ...
        TryCatch tryCatch = new TryCatch(tryContainer, new DefaultContainer());
        ...
        if (Throwable.class.isAssignableFrom(type)) {
            ...
            tryCatch.addHandler((Class<? extends Throwable>) type, variable, variablePointer, containerParser.parse(context, new DefaultContainer(), catchBody));
        }
    }
