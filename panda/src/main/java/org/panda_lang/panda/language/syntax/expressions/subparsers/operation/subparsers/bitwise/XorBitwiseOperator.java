...
public final class XorBitwiseOperator extends BitwiseOperation {

    @Override
    public RPNOperationAction<Number> of(Type returnType, int priority, Expression a, Expression b) {
        BiFunction<Number, Number, Number> xor = toFunction(priority);
    ...
    private BiFunction<Number, Number, Number> toFunction(int priority) {
        switch (priority) {
            ...
            case FLOAT:
                throw new PandaParserException("Cannot use xor operator on float type");
            case DOUBLE:
                throw new PandaParserException("Cannot use xor operator on double type");
            default:
