...
final class ArrayUtilsTest {
    ...
    void mergeArrays() {
        ...
        assertArrayEquals(merged, ArrayUtils.merge({ size -> new String[size] } as Function, ARRAY, ARRAY_WITH_NULL))
    }
    ...
    @Test
    void findIn() {
        assertTrue(ArrayUtils.findIn(ARRAY_WITH_NULL, value -> Objects.nonNull(value)).isPresent())
    }
    ...
    @Test
    void merge() {
        String[] merged = ArrayUtils.merge("-a", ARRAY, length -> new String[length])
