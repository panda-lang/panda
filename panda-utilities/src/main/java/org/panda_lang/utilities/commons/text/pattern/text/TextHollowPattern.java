...
import org.panda_lang.utilities.commons.text.pattern.PatternMatcher;
...
public final class TextHollowPattern implements PatternMatcher {
            ...
            if (hollowSymbol.isBasis()) {
                for (String variant : hollowSymbol.getVariants()) {
                    ...
                    if (variant.equals(part)) {
                        expression = expression.substring(variant.length());
                        break loop;
                    }
                }
                ...
                for (String variant : nextHollowSymbol.getVariants()) {
                    ...
                    expression = expression.substring(partIndex);
    ...
    public String getPart(String string, int beginIndex, int endIndex) {
        return string.length() >= endIndex ? string.substring(beginIndex, endIndex) : string.substring(beginIndex);
    }
